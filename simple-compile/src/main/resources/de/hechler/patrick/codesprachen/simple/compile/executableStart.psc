...
~IF #~ME
    ...
    #ME_ILLEGAL_MEM_POS -1
    #ME_DEP_LOAD_ERR_POS -1
    #ME_OUT_OF_MEM_ERR_POS -1
    #ME_DEP_LOAD_ERR_MSG_POS -1
    #ME_OUT_OF_MEM_ERR_MSG_POS -1
    #ME_START_ERR_MSG_POS -1
    #ME_ILLEGAL_MEM_ERR_MSG_POS -1
    #ME_ILLEGAL_MEM_ERR_MSG_LEN -1
~ELSE
    ~READ_SYM "[THIS]" #ADD~ME 1 #ME_ >
~ENDIF
    ...
    |> make stack grow-able
        ...
        #REL_POS ( ME_ILLEGAL_MEM_POS - --POS-- )
    ...
    |> add my interrupts
        ...
        #REL_POS ( ME_OUT_OF_MEM_ERR_POS - --POS-- )
        LEA [X01 + OFF], REL_POS
        #OFF ( INT_DEP_LOAD_ERR * 8 )
        #REL_POS ( ME_DEP_LOAD_ERR_POS - --POS-- )
        LEA [X01 + OFF], REL_POS
        MOV INTP, X01
        MOV INTCNT, INTERRUPT_COUNT
    |> call the main function
        |:
        |   func main m(num argc, char## argv) --> <num exitnum>
        |   
        |   struct main_function_arguments {
        |       num argc,
        |       char## argv
        |   }
        |   struct main_function_results {
        |       num exitnum
        |   }
        |:>
        MOV X00, 16
...
~ENDIF
    |> check if stack overflow
        ...
        JMPEQ stackOverflow
    |> @illegalMemory
        #REL_POS ( ME_ILLEGAL_MEM_ERR_MSG_POS - --POS-- )
        LEA X12, REL_POS
        #REL_POS ~DEL
        MOV X11, ME_ILLEGAL_MEM_ERR_MSG_LEN
        MOV X10, 9
        JMP fail
    @stackOverflow
        MOV X00, SP
...
~ENDIF
    #REL_POS ( ME_DEP_LOAD_ERR_MSG_POS - --POS-- )
    LEA X12, REL_POS
    #REL_POS ~DEL
    MOV X11, ME_DEP_LOAD_ERR_MSG_LEN
    MOV X10, 9
    JMP fail
~IF #~ME
...
~ENDIF
@outOfMem
    #REL_POS ( ME_OUT_OF_MEM_ERR_MSG_POS - --POS-- )
    LEA X12, REL_POS
    #REL_POS ~DEL
    MOV X11, ME_OUT_OF_MEM_ERR_MSG_LEN
    MOV X10, 8
    JMP fail
@startError
    #REL_POS ( ME_START_ERR_MSG_POS - --POS-- )
    LEA X12, REL_POS
    #REL_POS ~DEL
    MOV X11, ME_OUT_OF_MEM_ERR_MSG_LEN
    MOV X10, 4
