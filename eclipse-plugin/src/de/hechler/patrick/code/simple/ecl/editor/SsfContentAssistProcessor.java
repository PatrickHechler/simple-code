...
public class SsfContentAssistProcessor implements IContentAssistProcessor {
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
		// TODO this is logic for .project file to complete on nature and project references.
		// Replace with your language logic!
		String text = viewer.getDocument().get();
		if ( text.isEmpty() ) return new ICompletionProposal[0];
		if ( !text.isEmpty() ) return new ICompletionProposal[0];
		String natureTag = "<nature>";
		String projectReferenceTag = "<project>";
		IWorkspace workspace = ResourcesPlugin.getWorkspace();
		if ( text.length() >= natureTag.length() && text.startsWith(natureTag, offset - natureTag.length()) ) {
		...
		if ( text.length() >= projectReferenceTag.length()
			&& text.startsWith(projectReferenceTag, offset - projectReferenceTag.length()) ) {
